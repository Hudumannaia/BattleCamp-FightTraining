...
const float elemental_bonus[5][5]={
    { 1.0 , 0.75, 1.33, 1.0 , 1.0 },
...
bool rect_inside(SDL_Rect,int,int);
...
int main()
{
    ...
    SDL_Rect palya_hatter_helye   = { 0, windowH-15*z, 18*z, 15*z};///hatter elrendezes
    SDL_Rect healtbar_start       = { 0, 0,10,25};///a healtbar eleje
    SDL_Rect healtbar_mid         = { 9, 0, 2,25};///a healtbar maga (az adat)
    SDL_Rect healtbar_end         = {10, 0,10,25};///a healtbar vege
    SDL_Rect timer_start          = { 0, 0,10,25};///a timer eleje
    SDL_Rect timer_mid            = { 9, 0, 2,25};///a timer maga (az ido...)
    SDL_Rect timer_end            = {10, 0,10,25};///a timer vege
    SDL_Rect healtbar_start_helye = {                             0,16*z,                             z, z};///a healtbar elejenek a helye
    SDL_Rect healtbar_mid_helye   = {                             z,16*z,16*z*mezo.my_HP/mezo.my_MAX_HP, z};///a healtbar kozepenek a helye
    SDL_Rect healtbar_end_helye   = {17*z*mezo.my_HP/mezo.my_MAX_HP,16*z,                             z, z};///a healtbar vegenek a helye
    SDL_Rect timer_start_helye    = {                                 0,16*z,                                  z, z};///a timer elejenek a helye
    SDL_Rect timer_mid_helye      = {                                 z,16*z, 16*z*(SagaTime-clock()+saga_timer)/SagaTime, z};///a timer kozepenek a helye
    SDL_Rect timer_end_helye      = {17*z*(SagaTime-clock()+saga_timer)/SagaTime,16*z,                                  z, z};///a timer vegenek a helye
    SDL_Rect Ally_monster_helye[5];
    SDL_Rect Enemy_monster_helye[5];
    SDL_Rect Enemy_monster_healtbar_start[5];
    SDL_Rect Enemy_monster_healtbar_mid[5];
    SDL_Rect Enemy_monster_healtbar_end[5];
    ...
    SDL_Window* window = SDL_CreateWindow("Battle Camp - fight training", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, windowW, windowH, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    ...
    while(running)///a program magja
    {
        while(SDL_PollEvent(&event))///event kezeles
            switch(event.type){
                case SDL_QUIT:{///kilepes
                    running = false;
                    break;
                }
                case SDL_WINDOWEVENT:{///ablak atmeretezes
                    if(event.window.event==SDL_WINDOWEVENT_RESIZED){
                        ...
                        timer_mid_helye      = {                                 z,16*z, 16*z*(SagaTime-clock()+saga_timer)/SagaTime, z};///a timer kozepenek a helye NINCS NAGYON SZUKSEG RA
                        timer_end_helye      = {17*z*(SagaTime-clock()+saga_timer)/SagaTime,16*z,                                  z, z};///a timer vegenek a helye   NINCS NAGYON SZUKSEG RA
                        for(int i=0; i<5; i++){
                            Ally_monster_helye[i] = { i*18*z/5, 12*z, 18*z/5, 4*z};///baratsagos szornyek helyenek a bealitasa
                            Enemy_monster_helye[i] = { i*18*z/5, 6*z+(i%2)*z, 18*z/5, 5*z };///ellenseges szornyek helyenek a bealitasa
                            Enemy_monster_healtbar_start[i] = {              i*7*windowW/35, 10*z+(i%2)*z,                                 windowW/35, z};
                            Enemy_monster_healtbar_mid[i]   = { i*7*windowW/35 + windowW/35, 10*z+(i%2)*z, 5*windowW*Enemys[i].HP/Enemys[i].MAX_HP/35, z};
                            Enemy_monster_healtbar_end[i]   = { i*7*windowW/35 + 6*windowW*Enemys[i].HP/Enemys[i].MAX_HP/35, 10*z+(i%2)*z, windowW/35, z};
                        }
                ...
                case SDL_MOUSEMOTION:{///egermozgas
                    if(mezo.folyamat==BASIC or mezo.folyamat==SAGA)
                    {
                        ...
                        if(event.button.button==SDL_BUTTON_LEFT and mezo.folyamat==SAGA)
                        {
                            ...
                            if(x>z+z/2 and x<16*z+z/2)
                                mezo.elemek_helye[a][b].x = x-z-z/2;
                            if(y>18*z+z/2 and y<30*z+z/2)
                                mezo.elemek_helye[a][b].y = y-z-z/2;
                        }
                    }
                    break;
                }
                case SDL_MOUSEBUTTONDOWN:{///katintas
                    if(mezo.folyamat==BASIC){
                        bool target_change = false;
                        for(int i=0;  i<5; i++)
                        {
                            ///Enemy_monster_helye[i] = { i*18*z/5, 6*z+(i%2)*z, 18*z/5, 5*z };
                            if(rect_inside(Enemy_monster_helye[i],x,y) and Enemys[i].HP)///ha ez egyik szornyre (es el...) kattint akkor az targeteli :)
                            {
                                mezo.target = i;
                                target_change = true;
                                break;///nem muszaly
        ...
        switch(mezo.folyamat){///A jatek folyamatait szabalyozo resz
            ...
            case TEST:{
                ...
                for(int c=1; c<=mezo.combo_tmp; c++){
                    ...
                    if(szin==5)///szivecske
                        for(int i=0; i<5; i++)
                            mezo.my_heal += Allys[i].recovery*0.2*counter ;///a healeles kiszamolasa
            ...
            case ATTACK:{
                ...
                for(int i=0; i<5; i++)///minden szornyunket sorra vesszuk
                {
                    ...
                    if(Allys[i].HP>0 and Allys[i].precalculated_dmg>0)///tamad amelyik kell
                    {
                        if(mezo.AttackAll[i])///mindenkit kell tamadjon
                        {
                            ...
                            {
                                if(Enemys[j].HP>0){
                                    Enemys[j].HP -=(int)(Allys[i].precalculated_dmg*ComboBonus*elemental_bonus[Enemys[j].color][Allys[i].color]);
                                    if(Enemys[j].HP<0)
                                        Enemys[j].HP=0;
                                }
                            }
                            ...
                            /// az kivalasztott ellenfel eletebol levonunk a:  bonus[][] * ComboBonusz * gyujtott elem a szinben * szorny alap DMG / 3
                            Enemys[mezo.target].HP -=(int)(Allys[i].precalculated_dmg*ComboBonus*elemental_bonus[Enemys[mezo.target].color][Allys[i].color]);
                            if(Enemys[mezo.target].HP<0)
                                Enemys[mezo.target].HP=0;
                        }
                        if(Enemys[mezo.target].HP==0)///halott-e a tamadott ellenfel?
                            mezo.target = -1;
                    }
                    if(random_target)
                        mezo.target = -1;///ha igen uj ellenfelet tamadunk
                ...
                if(mezo.my_heal)
                {
                    ...
                    if(mezo.my_HP>mezo.my_MAX_HP)
                        mezo.my_HP = mezo.my_MAX_HP;
                    healtbar_start_helye = { 0, 16*z, z, z};///a healtbar elejenek a helyere rakaja
                    healtbar_mid_helye   = {z, 16*z, 16*z*mezo.my_HP/mezo.my_MAX_HP, z};///a healtbar kozepenek a helyere rakaja
                    healtbar_end_helye   = {17*z*mezo.my_HP/mezo.my_MAX_HP, 16*z, z, z};///a healtbar vegenek a helyere rakaja
                    mezo.my_heal = 0;
            ...
            case DEFEND:{
                ...
                {///tamadaszam randomizalas
                    ...
                    while(true)
                    {
                        ...
                        if(random==0 or tamadasok_szama==0 or tamadasok_szama==2*ellenfelek_szama)
                            break;
                    }
        ...
        if(clock()-fps_counter>1000/FPS){///ez a "frissites"
            ...
            SDL_RenderCopy(renderer, palya_hatter, NULL, &palya_hatter_helye);///jatek hatter bevagas
            for(int i=0; i<5; i++)
                for(int j=0; j<6; j++)
                    ///szines kovek bevagasa
                    if(mezo.elemek[i][j].szin>-1 and mezo.elemek[i][j].szin<6)
                        SDL_RenderCopy(renderer, runes_textures[mezo.elemek[i][j].szin], NULL, &mezo.elemek_helye[i][j]);

            SDL_RenderCopy(renderer, full_hatter, NULL, NULL);///a keret hatter
            for(int i=0; i<5; i++)
            {
                SDL_RenderCopy(renderer, Allies_textures[Allys[i].color], NULL,  &Ally_monster_helye[i]);///a "te" szornyeid berakasa
                if(Enemys[i].HP and running)
                {
                    ///az ellenseg berakasa az eletevel egyutt :DD de kiraly, ez nehez volt!!
                    SDL_RenderCopy(renderer,  Enemy_textures[Enemys[i].color], NULL, &Enemy_monster_helye[i]);
                    SDL_RenderCopy(renderer, Monster_healtbar[Enemys[i].color], &healtbar_start, &Enemy_monster_healtbar_start[i]);
                    SDL_RenderCopy(renderer, Monster_healtbar[Enemys[i].color],   &healtbar_mid,   &Enemy_monster_healtbar_mid[i]);
                    SDL_RenderCopy(renderer, Monster_healtbar[Enemys[i].color],   &healtbar_end,   &Enemy_monster_healtbar_end[i]);
                }
            ...
            if(saga_timer)
            {
                ...
                SDL_RenderCopy(renderer, full_timer, &timer_start, &timer_start_helye);///a timer elejenek beszurasa
                SDL_RenderCopy(renderer, full_timer,   &timer_mid,   &timer_mid_helye);///a timer kozepenek beszurasa
                SDL_RenderCopy(renderer, full_timer,   &timer_end,   &timer_end_helye);///a timer vegenek beszurasa
            }
            else
            {
                SDL_RenderCopy(renderer, full_healtbar, &healtbar_start, &healtbar_start_helye);///a healtbar elejenek beszurasa
                SDL_RenderCopy(renderer, full_healtbar,   &healtbar_mid,   &healtbar_mid_helye);///a healtbar kozepenek beszurasa
                SDL_RenderCopy(renderer, full_healtbar,   &healtbar_end,   &healtbar_end_helye);///a healtbar vegenek beszurasa
            }
            ...
            if(event.button.button==SDL_BUTTON_LEFT and mezo.folyamat==SAGA and mezo.elemek[a][b].szin>-1 and mezo.elemek[a][b].szin<6)
                SDL_RenderCopy(renderer, runes_textures[mezo.elemek[a][b].szin], NULL, &mezo.elemek_helye[a][b]);
...
bool rect_inside(SDL_Rect hatarok,int x, int y){
